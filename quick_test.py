...
def reader_parser(path, files, queue, *args):
    list_for_df = []
    for file in files:
        file=open(path+"/"+file)
...
def parallel_parse(path, files, *args):
    # Set number of CPUs
    ...
    for w in range(num_cpus):
        ...
        if right>num_files:
            right=num_files
        process = Process(target=reader_parser, args=(path, files[left:right], queue, *args))
        process.start()
        right = left
...
if __name__ == '__main__':
    ...
    evidence = parallel_parse("data/evidence", files_evidence, "targetId", "diseaseId", "score")
    target = parallel_parse("data/targets", files_targets, "id", "approvedSymbol")
    disease = parallel_parse("data/diseases", files_diseases, "id", "name")
